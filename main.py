...
with open(ROOT/"resources/appdata.json", mode="r") as fp:
...
try:
    with open(ROOT/"resources/fannet.json", 'r') as fp:
        NET_F = model_from_json(fp.read())
    with open(ROOT/"resources/colornet.json", 'r') as fp:
        NET_C = model_from_json(fp.read())
    
    NET_F.load_weights(ROOT/"resources/fannet_weights.h5")
    NET_C.load_weights(ROOT/"resources/colornet_weights.h5")
except:
    RUN_FLAG = False
